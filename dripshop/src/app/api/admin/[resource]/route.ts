...
const schemas={categories:base.extend({parent_id:z.string().uuid().nullable().optional()}),collections:base.extend({is_active:z.boolean()}),creators:base.extend({bio:z.string().max(5000).optional(),verified:z.boolean().optional()}),featured_categories:z.object({title:z.string().min(2),image_url:image,link_url:z.string().regex(/^\/(?!\/)/),button_text:z.string().min(1),position:z.number().int().min(0),is_active:z.boolean()}),store_settings:z.object({name:z.string().min(2),support_email:z.string().email().or(z.literal('')),free_shipping_threshold:z.number().min(0),standard_shipping:z.number().min(0),express_shipping:z.number().min(0)})};
const tables=['products','categories','collections','creators','featured_categories','store_settings','orders','profiles','contact_messages','newsletter_subscribers'];
...
export async function POST(request:NextRequest,{params}:{params:Promise<{resource:string}>}){try{const {db}=await requireUser(true);const {resource}=await params;const body=await request.json();if(resource==='products'){
 ...
 const {data,error}=await query.select().single();if(error)throw new Error('Não foi possível salvar. Verifique os dados e o slug.');return successResponse(data);
