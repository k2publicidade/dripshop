...
export default function FavoriteButton({productId}:{productId:string}) {
 ...
 return <div><button type="button" disabled={busy} onClick={toggle} aria-pressed={saved} className="flex items-center gap-2 text-sm py-3"><Heart size={19} fill={saved?'currentColor':'none'}/>{saved?'Salvo nos favoritos':'Salvar nos favoritos'}</button>{error&&<p role="alert" className="text-sm text-gray-700">{error}</p>}</div>;
}
